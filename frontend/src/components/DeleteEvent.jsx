...
import { ArrowLeft, Calendar, Delete, Users } from 'lucide-react';
import { useParams, useNavigate } from 'react-router-dom';
import { fetchEventsByAdmin, deleteEvent } from '../services/eventAPI';
  ...
  const EventCard = ({ event }) => (
    <div className="bg-[#1a1a1a] rounded-xl overflow-hidden shadow-lg border border-[#A3CFF0] hover:shadow-xl transition-all duration-300 flex flex-col max-w-xs mx-auto">
      ...
      <div className="p-4 flex flex-col items-center text-center">
        ...
        <button 
          className="px-4 py-2 bg-[#A3CFF0] bg-opacity-30 text-white rounded-lg hover:bg-opacity-40 transition-colors duration-300 w-full"
          onClick={() => navigate(`/delete-events/${event._id}`)}
        >
          View Details
        </button>
      </div>
      ...
      <div className="min-h-screen bg-[#1a1a1a] p-8">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="flex flex-col lg:flex-row gap-6">
            ...
            {/* Right Column - Contact Info and Delete Button */}
            <div className="w-full lg:w-1/4">
              <div className="mb-6">
              ...
              {/* Delete Button */}
              <button 
                className="w-full bg-red-600 text-white py-3 px-6 rounded-lg font-bold hover:bg-red-700 transition-colors flex items-center justify-center gap-2"
